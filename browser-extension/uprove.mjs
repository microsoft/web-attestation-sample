...
var cryptoECCExports = {};
var cryptoECC$1 = {
    get exports() { return cryptoECCExports; },
    set exports(v) { cryptoECCExports = v; },
};

var utilitiesExports = {};
var utilities = {
    get exports() { return utilitiesExports; },
    set exports(v) { utilitiesExports = v; },
};
...
var cryptoMathExports = {};
var cryptoMath = {
    get exports() { return cryptoMathExports; },
    set exports(v) { cryptoMathExports = v; },
};
...
class IssuerParams {
     ...
     * @constructor
     */
    constructor(UIDP, descGq, UIDH, g, e, S) {
        this.UIDP = UIDP;
        this.descGq = descGq;
        this.UIDH = UIDH;
        this.g = g;
        ...
        _IssuerParams_P.set(this, void 0);
        this.Gq = new Group(this.descGq);
        ...
        hash.update(this.e);
        hash.update(this.S);
        __classPrivateFieldSet(this, _IssuerParams_P, hash.digest(), "f");
    ...
    */
    static async create(UIDP, descGq, UIDH, g, e, S) {
        ...
        parameters.P = await __classPrivateFieldGet(parameters, _IssuerParams_P, "f");
        return parameters;
    }
 ...
 * @export
 * @class Prover
 * @extends {IssuanceParticipant}
 */
class Prover extends IssuanceParticipant {
    constructor(ip, A, TI, PI, n) {
        super(n);
        this.ip = ip;
        this.A = A;
        this.TI = TI;
    ...
    static async create(ip, A, TI, PI, n) {
        const prover = new Prover(ip, A, TI, PI, n);
     ...
     *
     *
     * @param {FirstIssuanceMessage} msg1
     * @return {*}  {Promise<SecondIssuanceMessage>}
     * @memberof Prover
     */
...
class Issuer extends IssuanceParticipant {
    ...
    createThirdMessage(msg2) {
        ...
        const Zq = this.Gq.Zq;
        const sigmaR = this.w.map((w_i, i) => Zq.add(Zq.mul(msg2.sC[i], this.y0), w_i));
        return {
            sR: sigmaR
        };
    }
 ...
 * @param {ECGroup} descGq
 * @param {number} n
 * @param {(Byte[] | undefined)} [e=undefined]
 * @param {Uint8Array} [S=new Uint8Array()]
 * @param {(IssuerKeyPair | undefined)} [issKeyPair]
 * @param {(Uint8Array | undefined)} [UIDP]
 * @return {*}  {Promise<IssuerKeyAndParams>}
 */
async function createIssuerKeyAndParams(descGq, n, e = undefined, S = new Uint8Array(), issKeyPair, UIDP) {
    ...
    if (!UIDP) {
        // UIDP not defined, let's set it to the hash of the other fields
...
async function generatePresentationProof(ip, D, upkt, m, A, md = new Uint8Array()) {
    ...
    const Gq = ip.Gq;
    const Zq = Gq.Zq;
    const x = await Promise.all(A.map(async (a, i) => await computeXi(i + 1, ip, a)));
    const w0 = Zq.getRandomElement();
    const w = Zq.getRandomElements(n - D.length);
    const H = Gq.getHash();
    const a = await H.digest(Gq.multiModExp([upkt.upt.h, ...ip.g.slice(1, n + 1).filter((g, i) => U.includes(i + 1))], [w0, ...w]));
...
async function verifyPresentationProof(ip, upt, m, pp, md = new Uint8Array()) {
    ...
    await verifyTokenSignature(ip, upt);
    // presentation proof verification
    const xt = await computeXt(ip, upt.TI);
    let D = [];
    let x = [];
    if (pp.A) {
        x = await Promise.all(Object.entries(pp.A).map(([iStr, Ai]) => {
            const i = parseInt(iStr);
            D.push(i);
            return computeXi(i, ip, Ai);
        }));
    ...
    const base0 = Gq.multiModExp([ip.g[0], ...ip.g.filter((g, i) => D.includes(i)), ip.g[t]], [Zq.ONE, ...x, xt]);
    const hashInput = Gq.multiModExp([base0, upt.h, ...ip.g.slice(1, t).filter((g, i) => !D.includes(i + 1))], [Zq.negate(challengeData.c), pp.r[0], ...pp.r.slice(1)]);
...
class FieldZqElement {
    ...
    getBytes() {
        const bytes = cryptoMathExports.digitsToBytes(this.scalar.m_digits);
...
class FieldZq {
    constructor(q) {
        this.q = q;
        const qBytes = cryptoMathExports.digitsToBytes(q);
        this.elementLength = qBytes.length;
        this.Zq = new cryptoMathExports.IntegerGroup(qBytes);
    ...
    getElement(encoded) {
        let digits = cryptoMathExports.bytesToDigits(Array.from(encoded));
        // Check size of the new element
        const result = cryptoMathExports.intToDigits(0);
        while (cryptoMathExports.compareDigits(digits, this.q) >= 0) {
            // too big, reduce (will only call once)
            cryptoMathExports.subtract(digits, this.q, result); // could I replace result with digits? TODO
            digits = result;
        }
    ...
    getRandomElement(nonZero = false) {
        let done = false;
        let randZq = cryptoMathExports.Zero;
        while (!done) {
            const ranBytes = crypto$1.getRandomValues(new Uint8Array(this.elementLength));
            randZq = cryptoMathExports.bytesToDigits(Array.from(ranBytes));
            if (cryptoMathExports.compareDigits(randZq, this.q) < 0) {
                done = true;
            }
            if (nonZero && cryptoMathExports.isZero(randZq)) {
                done = false;
            }
        }
...
class GroupElement {
    ...
    getBytes() {
        const encoded = cryptoECCExports.sec1EncodingFp().encodePoint(this.point);
...
const P256Curve = cryptoECCExports.createCurve(CurveNames.P256);
const P384Curve = cryptoECCExports.createCurve(CurveNames.P384);
const P521Curve = cryptoECCExports.createCurve(CurveNames.P521);
class Group {
    constructor(descGq) {
        if (descGq == ECGroup.P256) {
        ...
        this.ecOperator = cryptoECCExports.EllipticCurveOperatorFp(this.curve);
    ...
    updateHash(H) {
        // H(p,a,b,g,q,1)
        H.update(new Uint8Array(cryptoMathExports.digitsToBytes(this.curve.p)));
        H.update(new Uint8Array(cryptoMathExports.digitsToBytes(this.curve.a)));
        H.update(new Uint8Array(cryptoMathExports.digitsToBytes(this.curve.b)));
        H.update(this.g /*new GroupElement(this.curve.generator).getBytes()*/);
        H.update(new Uint8Array(cryptoMathExports.digitsToBytes(this.curve.order)));
        H.update(new Uint8Array([1]));
    }
    parsePoint(x, y) {
        const point = new cryptoECCExports.EllipticCurvePointFp(this.curve, false, cryptoMathExports.bytesToDigits(x), cryptoMathExports.bytesToDigits(y));
        return new GroupElement(point);
    }
    getElement(encoded) {
        return new GroupElement(cryptoECCExports.sec1EncodingFp().decodePoint(Array.from(encoded), this.curve));
    }
    getIdentity() {
...
function parseSpecification(S) {
    const spec = JSON.parse(S.toString());
    return spec;
}
async function createIssuerKeyAndParamsUPJF(descGq, specification, issKeyPair) {
    ...
    return await createIssuerKeyAndParams(descGq, n, undefined, Buffer.from(JSON.stringify(specification)), issKeyPair, undefined);
}
var UPAlg;
(function (UPAlg) {
    UPAlg["UP256"] = "UP256";
...
function encodePrivateKeyAsBase64Url(y0) {
...
async function decodeJWKAsIP(jwk) {
    ...
    return await IssuerParams.create(fromBase64Url(jwk.kid), descGq, groupToHash(descGq), g, e.map(e => new Byte(e)), SBytes);
}
function parseTokenInformation(TI) {
    const tokenInformation = JSON.parse(TI.toString());
...
var lib = { uprove, serialization, upjf, Buffer, Byte };

export { lib as default };
//# sourceMappingURL=uprove.mjs.map
