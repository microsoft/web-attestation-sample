...
void (async () => {
    try {
        ...
        if (!uwa) {
            throw "missing uwa string";
        }
        if (!uwa.startsWith(uwaScheme)) {
            // not a uwa string  
            throw "invalid uwa string; must start with " + uwaScheme;
        }
        ...
        if (!issuerParamsJWK) {
            ...
            throw "issuer params not found for kid: " + kid;
        };
        ...
        try {
            ...
            console.log("presentation proof verification failed: " + err);
        }
